...
export async function openAttachment(
    ...
    try {
    const recentAttachment = await workerBridge.dbHelper.getAttachmentItem(libraryID, key);
    if (recentAttachment) {
        await services.addRecentItem({
            libraryID, key,
            name: recentAttachment.raw.data.filename || recentAttachment.title || key,
            itemType: "attachment",
            contentType: recentAttachment.raw.data.contentType,
        });
    }
    } catch (error) {
        services.logService.warn("Could not record recent attachment", "Viewer", error);
    }
