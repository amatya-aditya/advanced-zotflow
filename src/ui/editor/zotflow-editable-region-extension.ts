...
/** Read the editable regions from an editor state, tolerating version skew. */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getEditableRegions(state: any): EditableRegion[] {
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    return (state.field(editableRegionsField as any, false) ??
        []) as EditableRegion[];
}
...
const editableRegionSyncPlugin = ViewPlugin.fromClass(
    class {
        ...
        update(update: ViewUpdate) {
            ...
            // Cast through `any` to avoid @codemirror/state version skew when
            // @codemirror/view installs its own nested state package.
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            const regions = update.state.field(
        ...
        private scheduleSync(
            ...
            // `update.state` can come from a nested @codemirror/state copy.
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            state: any,
        ) {
