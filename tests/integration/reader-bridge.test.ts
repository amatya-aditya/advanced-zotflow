...
describe("handshake", () => {
    it("connects through the frame element without a Penpal handshake", async () => {
        const h = harness();
        const frame = h.iframe as unknown as {
            __OBSIDIAN_BRIDGE__?: () => Awaited<ReturnType<typeof h.shakehand>>;
        };
        const { token, register } = frame.__OBSIDIAN_BRIDGE__!();
        await register(h.child, token);
        await h.connecting;
        expect(h.bridge.state).toBe("bridge-ready");
        await h.bridge.initReader(makeReaderOptions());
        expect(h.child.initReader).toHaveBeenCalledOnce();
        await h.bridge.dispose();
        expect(frame.__OBSIDIAN_BRIDGE__).toBeUndefined();
        await expect(register(h.child as never, token)).rejects.toThrow("expired connection");
    });
...
describe("handleSetDataTransferAnnotations", () => {
    ...
    it("preserves rich annotation callouts when a source note exists", async () => {
        state.notesByKey.set("PARENTKEY", "Sources/Paper.md");
        const h = harness();
        const parent = await h.completeHandshake();
        const dt = createFakeDataTransfer();
        ...
        expect(dt.data.get("text/plain")).toContain("> [!zotflow-highlight-#ffd400]");
        expect(dt.data.get("text/plain")).toContain("> > first\n^a");
        expect(dt.data.get("text/plain")).toContain("> > second\n^b");
        expect(dt.data.get("text/plain")).toContain("key=ATTACHKEY");
    });
});
