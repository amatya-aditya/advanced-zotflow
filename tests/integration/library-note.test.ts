...
function placeNote(
    ...
    host.frontmatter.set(path, { "library-id": LIB, ...frontmatter });
}
...
describe("ensureNotePath", () => {
    ...
    test("a stale index entry recreates the missing note safely", async () => {
        await seedArticle();
        host.keyIndex.set("PARENT01", "Source/@PARENT01.md");
        expect(await service.ensureNotePath(LIB, "PARENT01")).toBe("Source/@PARENT01.md");
        expect(host.vault.get("Source/@PARENT01.md")).toContain("zotero-key");
    });
