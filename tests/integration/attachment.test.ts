...
describe("MD5 integrity", () => {
    ...
    it("keeps the server digest when a WebDAV copy disagrees", async () => {
        ...
        h = await createAttachmentHarness({ settings: { useWebDav: true, webDavVerified: true, webDavUrl: "https://dav.example/zotero/", webDavUser: "user", webdavpassword: "password" } });
        const item = await h.seedAttachment("ATTACH01", {
